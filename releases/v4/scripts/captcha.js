...
(function (S) {
  ...
  // ═══════ Kiểm tra Captcha đang mở trên màn hình ═══════
  function isCaptchaOpen() {
    const taiLieu = layTaiLieuGame();
    for (const doc of taiLieu) {
      if (!doc || !doc.body) continue;

      // 1. Kiểm tra Grid 16 ô Stop the Goblins / Skeleton / Moon Seeker / Zombie
      ...
      for (const w of wraps) {
        ...
        if (cellCount >= 16) return true;
      }

      // 2. Kiểm tra Rương kho báu (ảnh transform hoặc img.w-16)
      const ruongImg = doc.querySelector("img.w-16, img[style*='transform'][style*='perspective']");
      if (ruongImg && xemPhanTuRanh(ruongImg)) return true;

      // 3. Kiểm tra text cảnh báo hoặc tiêu đề Captcha / Reward
      const cacNhan = doc.querySelectorAll("h1, h2, h3, p, span, button");
      for (const n of cacNhan) {
        if (!xemPhanTuRanh(n)) continue;
        const txt = (n.textContent || "").trim().toLowerCase();
        if (txt.includes("tap the chest") || txt.includes("chest to open") ||
            txt.includes("stop the goblins") || txt.includes("moon seeker") ||
            txt.includes("stop the skeletons") || txt.includes("skeleton") ||
            txt.includes("zombie") || txt.includes("skeletons") ||
            txt.includes("are you a human") || txt.includes("verify you are human") ||
            txt.includes("clock is out of sync") || txt.includes("unclaimed reward") ||
            txt.includes("claim reward") || txt.includes("claim prize") || txt.includes("treasure chest")) {
          return true;
        }
      }
    }
